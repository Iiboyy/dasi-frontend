...
export default function ProductDetail({ product, onNavigate, previousPage }) {
  ...
  const handleAddToCart = async () => {
    ...
    if (result?.success) {
      ...
      showToast({ message: result?.message || "Gagal menambahkan", type: "error" });
    }
  };
  ...
  const handleBuyNow = async () => {
    ...
    if (result?.success) {
      ...
      showToast({ message: result?.message || "Gagal, coba lagi", type: "error" });
    }
  };
  ...
  return (
    <div className="w-full min-h-[calc(100vh-64px)] bg-gray-50 px-10 py-10 pb-16">
      <div className="flex items-center gap-3 mb-7">
        <button onClick={() => onNavigate(previousPage || "home")}
        ...
        <div className="flex items-center gap-1.5 text-sm text-gray-400">
          <button onClick={() => onNavigate("home")}
            className="hover:text-blue-600 transition-colors bg-transparent border-none cursor-pointer text-sm text-gray-400">
            Beranda
          </button>
          <span>/</span>
          <span className="text-gray-700 font-medium truncate max-w-50">{product.name}</span>
        </div>
      </div>

      <div className="flex gap-8 flex-wrap items-start">
        {/* Gambar */}
        <div className="flex-[0_0_340px] bg-white rounded-2xl border border-gray-200 flex items-center justify-center h-80 shadow-sm overflow-hidden">
          {product.thumbnail ? (
        ...
        <div className="flex-1 min-w-75">
          <div className="bg-white rounded-2xl border border-gray-200 p-7">
            ...
            <div className="mb-5">
              ...
              <p className="text-xs text-gray-400 capitalize">Kategori: {product.category}</p>
            </div>
            ...
            <div className="mb-7">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-base font-bold min-w-8 text-center">{qty}</span>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleBuyNow} disabled={loading}
                className={`flex-1 py-3 rounded-xl text-sm font-bold border-none transition-colors ${loading ? "bg-blue-300 cursor-not-allowed text-white" : "bg-blue-600 text-white cursor-pointer hover:bg-blue-700"}`}>
